<template>
    ...
    <el-table :data="tableData" stripe style="width: 100%">
      ...
      <el-table-column prop="action" label="操作" width="300" fixed="right">
        <template v-slot="scope">
          <el-button type="success" @click="handleDialogValue(scope.row.id)"
            >详情</el-button
          >
          <el-button type="primary" @click="handleOrderStatus(scope.row.id, 2)"
            >发货</el-button
          >
          <el-button type="primary" @click="handleOrderStatus(scope.row.id, 3)"
            >退货</el-button
          >
          <el-button
            type="danger"
  ...
  <Dialog v-model="dialogVisible" :id="productId"></Dialog>
</template>
<script setup>
...
const handleOrderStatus = (id, status) => {
  ElMessageBox.confirm('您确定要更新这个订单状态吗？', '系统提示', {
    ...
    .then(async () => {
      const res = await axios.post('admin/order/updateStatus', {
        id: id,
        status: status
      })
      if (res.data.code === 0) {
...
const handleDelete = (id) => {
  ElMessageBox.confirm('您确定要删除这条记录吗？', '系统提示', {
    ...
    .then(async () => {
      const res = await axios.get('admin/order/delete/' + id)
      if (res.data.code === 0) {
        ElMessage({
          type: 'success',
          message: '删除成功'
